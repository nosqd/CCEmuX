...
public class Font {
	...
	public static class CustomFont extends Font {
		...
		@Override
		public int addChar(String c, int[] d) throws TMLuaException {
			char cIn = c.charAt(0);
			for(int i = 0;i<chars2.length();i++){
				if(chars2.charAt(i) == cIn)throw new TMLuaException("Character already exists in sprite");
			}
			for(int i = 0;i<chars2.length();i++){
			...
			throw new TMLuaException("Sprite is full");
		}
		@Override
		public void remove(String c) throws TMLuaException {
			char cIn = c.charAt(0);
			for(int i = 0;i<chars2.length();i++){
			...
			throw new TMLuaException("Character not found");
		}
		@Override
		public void clear() throws TMLuaException {
			Arrays.fill(widths, (byte) 1);
			chars = new int[widths.length][fontHeight];
			this.chars2 = String.join("", Collections.nCopies(widths.length, "\u0000"));
		}
	...
	public void clear() throws TMLuaException {
		throw new TMLuaException("Selected font is not modifiable");
	}
