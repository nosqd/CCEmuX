...
public interface TriBuilder {
	...
	public void finish() throws TMLuaException;

	static TriBuilder builder(int mode, List<Triangle> list, Supplier<Matrix4d> mm, IntSupplier boundTexID) throws TMLuaException {
		switch (mode) {
		case GLConstants.GL_TRIANGLES:
			return new Default(list, mm, boundTexID);

		default:
			throw new TMLuaException("Unknown build mode: " + mode);
		}
	}
	...
	public static class Default implements TriBuilder {
		...
		@Override
		public void finish() throws TMLuaException {
			if(p != -1 && p != 2)throw new TMLuaException("Incomplete triangle");
			buildTri();
		}
